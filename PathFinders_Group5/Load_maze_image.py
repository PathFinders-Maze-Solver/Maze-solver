...
def setup(parent_frame):
    ...
    class MazeProcessor:
        ...
        def _detect_walls_cell_based(self, maze_binary, rows, cols, cell_size):
            """
            Identify walls by looking for wall segments in full cells and figuring out which edges the walls are closest to.
    ...
    def load_maze_image():
            global cols, rows, grid, w, x_offset, y_offset, start, goal
    ...
    def solve_maze_selected():
        if algorithm_var.get() == "BFS":
            ...
            solve_maze_dijkstra(start, goal, grid, index, canvas, execution_time_label, root, width, height, w, x_offset,
                                y_offset)
